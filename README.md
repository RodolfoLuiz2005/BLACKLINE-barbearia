...
Essas variaveis sao do backend legado. O painel atual usa modo local de teste com a senha `blackline2025`; antes de publicar para cliente real, substitua esse modo por Firebase Auth.
...
Na pagina publica, a funcao `testeFirestore()` fica disponivel no console do navegador apenas em `localhost`, `127.0.0.1` ou arquivo local, para validar a conexao com o Firestore. Ela cria um agendamento de teste em `agendamentos` e lista a quantidade total de documentos. Nao use essa funcao como fluxo de producao.

### Proximo passo de seguranca

Para vender ou publicar em producao, troque o login local de teste do ADM por Firebase Auth e ajuste as regras do Firestore para permitir leitura, atualizacao e exclusao apenas para administradores autenticados.
...
>>>>>>> a30cdbd (atualiza projeto blackline)
